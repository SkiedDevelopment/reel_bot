...
async def addreel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as s:
        ...
        expected = acc[0]
        if sup:
            if sup.lower() != expected.lower():
                return await update.message.reply_text(f"🚫 That reel isn't from @{expected}.")
        else:
            try:
                post = Post.from_shortcode(Instaloader().context, code)
            except Exception:
                return await update.message.reply_text("❌ Couldn't fetch reel data.")
            if post.owner_username.lower() != expected.lower():
                return await update.message.reply_text(f"🚫 That reel belongs to @{post.owner_username}.")
