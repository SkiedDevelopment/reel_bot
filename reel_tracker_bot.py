...
async def loginstatus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if INSTALOADER_SESSION.context.is_logged_in:
        user = INSTALOADER_SESSION.context.username
        await update.message.reply_text(f"✅ IG session active as @{user}")
    else:
...
async def addaccount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await log_to_group(
        context.bot, f"Admin @{user.username} assigned {hdl} to {tgt}"
    )
...
async def removeaccount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as s:
        res = await s.execute(
            text(
                "DELETE FROM user_accounts WHERE "
                "user_id=:u AND insta_handle=:h RETURNING *"
            ),
            {"u": int(tgt), "h": hdl},
        )
        await s.commit()
    if res.rowcount:
        ...
        await log_to_group(
            context.bot, f"Admin @{user.username} removed {hdl} from {tgt}"
        )
    else:
...
async def userstats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as s:
        ...
        rres = await s.execute(
            text("SELECT id,shortcode FROM reels WHERE user_id=:u"), {"u": tgt}
        )
        reels = rres.fetchall()
    total, details = 0, []
    for rid, sc in reels:
