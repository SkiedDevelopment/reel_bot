...
async def init_db():
    async with engine.begin() as conn:
        ...
        # migrations: add missing columns/tables
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cmds = []
    cmds.append("👋 <b>Welcome to Reel Tracker Bot!</b>")
    cmds.append("")
    cmds.append("📋 <b>Available Commands:</b>")
    cmds.append("• <code>/addreel &lt;link&gt;</code> – Add a reel to track")
    cmds.append("• <code>/removereel &lt;shortcode&gt;</code> – Remove your reel")
    cmds.append("• <code>/myreels</code> – List your tracked reels")
    cmds.append("• <code>/stats</code> – Your stats")
    if is_admin(update.effective_user.id):
        cmds.append("• <code>/addaccount &lt;userid&gt; &lt;@handle&gt;</code> – Allow user’s IG account")
        cmds.append("• <code>/removeaccount &lt;userid&gt;</code> – Revoke allowed IG account")
        cmds.append("• <code>/forceupdate</code> – Force update all reels")
        cmds.append("• <code>/checkapi</code> – API health check")
        cmds.append("• <code>/leaderboard</code> – Global leaderboard")
    cmds.append("")
...
async def addreel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with AsyncSessionLocal() as session:
        ...
        handle = row[0]
        # optional: verify link contains that handle? skipped for brevity
...
async def myreels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = ["🎥 <b>Your Reels:</b>"]
    for sc in reels:
        msg.append(f"• <a href=\"https://www.instagram.com/reel/{sc}/\">{sc}</a>")
    await update.message.reply_text("\n".join(msg),
                                    parse_mode=ParseMode.HTML,
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for sc, v in top_reels:
        msg.append(f"• <a href=\"https://www.instagram.com/reel/{sc}/\">{sc}</a> – {v}")
    await update.message.reply_text("\n".join(msg),
                                    parse_mode=ParseMode.HTML,
                                    disable_web_page_preview=True)
...
async def leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uname, vids, views in data:
        msg.append(f"• {uname or '—'} – {vids} vids – {views} views")
...
async def forceupdate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prog = await update.message.reply_text("🔄 Updating...")
    async with AsyncSessionLocal() as session:
        ...
        await session.commit()
    await prog.edit_text(f"✅ Updated {cnt} reels.")

@debug_handler
async def checkapi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return await update.message.reply_text("🚫 <b>Unauthorized.</b>", parse_mode=ParseMode.HTML)
    v = await scrape_instagram_reel_views("Cx9L5JkNkfJ")  # sample
    if v >= 0:
...
def main():
    ...
    # start DB migrations and tables
